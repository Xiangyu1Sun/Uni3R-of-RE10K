...
def checkpoint_filter_fn(
    ...
    for k, v in state_dict.items():
        if 'aggregator' in k:
            if 'patch_embed.patch_embed.proj.weight' in k:
                O, I, H, W = model.aggregator.patch_embed.patch_embed.proj.weight.shape
            ...
            # TODO: check if the sampling method for pos embedding is right
            # if 'patch_embed.pos_embed' in k:
            #     # import pdb; pdb.set_trace()
            #     O, I, C = model.aggregator.patch_embed.pos_embed.shape
            #     if (v.shape[1]-1) != I-1:
            #         I_sqrt = int(math.sqrt(I-1))
            #         v_sqrt = int(math.sqrt(v.shape[1]-1))
            #         v_input = v[:, 1:, :]
            #         v_input = v_input.view(1,v_sqrt,v_sqrt,C).permute(3,0,1,2)
            #         v_output = resample_patch_embed(
            #             v_input,
            #             (I_sqrt, I_sqrt),
            #             interpolation=interpolation,
            #             antialias=antialias,
