    ...
    def __iter__(self):
        ...
        for chunk_path in self.chunks:
            ...
            for example in chunk:
                ...
                try:
                    context_indices, target_indices, overlap, support_indices = self.view_sampler.sample(
                        scene,
                        extrinsics,
                        intrinsics,
                    )
                ...
                try:
                    ...
                    support_images = [
                        example["images"][index.item()] for index in support_indices
                    ]
                    support_images = self.convert_images(support_images)
                except IndexError:
                    continue
                except OSError:
                ...
                support_image_invalid = support_images.shape[1:] != (3, *self.cfg.original_image_shape)
                if self.cfg.skip_bad_shape and (context_image_invalid or target_image_invalid or support_image_invalid):
                    print(
                ...
                example = {
                    ...
                    "support": {
                        "extrinsics": extrinsics[support_indices],
                        "intrinsics": intrinsics[support_indices],
                        "image": support_images,
                        "near": self.get_bound("near", len(support_indices)) / scale,
                        "far": self.get_bound("far", len(support_indices)) / scale,
                        "index": support_indices,
                        "overlap": overlap,
                    },
                    "target": {
