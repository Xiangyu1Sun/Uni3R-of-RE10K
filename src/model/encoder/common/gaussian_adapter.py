...
class UnifiedGaussianAdapter(GaussianAdapter):
    def forward(
        ...
        # scale initialization from noposplat for canonical space
        scales = 0.001 * F.softplus(scales)
        scales = scales.clamp_max(0.3)

        # scale initialization from GS-LRM
        # scales = (scales - 2.3).float()
        # scales = (torch.exp(scales)).clamp(max=0.3)
        ...
        if input_images is not None:
            # B,V,H*W,1,1,3
            input_images = inverse_normalize(input_images)
